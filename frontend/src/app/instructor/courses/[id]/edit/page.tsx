...
export default function EditCourse({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchCourse = async () => {
    try {
      const res = await api.get(`/courses/${unwrappedParams.id}`);
      if (res.data.success) {
        const c = res.data.data;
        setTitle(c.title || '');
        setDescription(c.description || '');
        setImageUrl(c.imageUrl || '');
        setCategory(c.category || CATEGORIES[0]);
        setWhatYouWillLearn(c.whatYouWillLearn?.length ? c.whatYouWillLearn : ['']);
        setLessons(c.lessons?.length ? c.lessons : [{ title: '', videoUrl: '' }]);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.put(`/courses/${unwrappedParams.id}`, {
        title,
        description,
        category,
